...
function SelectProvider() {
  ...
  useEffect(() => {
    ...
    const fetchProviders = async () => {
      try {
        ...
        if (e.name !== "AbortError") { console.error(e); setCurrentProvider(null); }
      } finally {
  ...
  return (
    <PageCard>
      <Typography variant="h4" gutterBottom>Select Primary Care Provider</Typography>

      <Box sx={{ mb: 2 }}>
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Typography>Current provider:</Typography>
          {loadingCurrentProvider || isLoading ? <CircularProgress size={20} /> : (
        ...
        {!loadingCurrentProvider && currentProvider?.selectionTime && (
